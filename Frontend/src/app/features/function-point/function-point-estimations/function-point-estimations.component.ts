...
} from '../../../core/models/function-point/function-point.models';

@Component({
  selector: 'app-function-point-estimations',
  standalone: true,
  imports: [
    CommonModule, 
    NavbarComponent, 
    SelectProjectDialogComponent,
    FunctionPointEstimationDialogComponent
  ],
  template: `
    ...
    <div class="container">
      <div class="header">
        <div class="header-content">
          ...
          <h1>Estimaciones de Puntos de Función</h1>
          <p class="subtitle">Gestión de estimaciones basadas en Function Points</p>
        </div>
      ...
      <div *ngIf="!loading() && !error() && estimations().length === 0" class="empty-state">
        ...
        <h2>Sin estimaciones de Puntos de Función</h2>
        ...
        <table class="estimations-table">
          ...
          <tbody>
            <tr *ngFor="let estimation of estimations()" class="estimation-row">
              ...
              <td>{{ formatDecimal(estimation.unadjustedFp) }}</td>
              <td>
                <span class="afp-badge">{{ formatDecimal(estimation.adjustedFp) }}</span>
              </td>
              <td>
                <span [class]="'complexity-badge ' + estimation.complexityLevel.toLowerCase()">
                  {{ getComplexityLabel(estimation.complexityLevel) }}
                </span>
              </td>
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            ...
            <div class="stat-content">
              ...
              <div class="stat-label">Total Puntos de Función</div>
            </div>
          </div>
          <div class="stat-card">
            <div class="stat-icon">👥</div>
    ...
    <!-- Estimation Dialog -->
    <app-function-point-estimation-dialog
    ...
    <div *ngIf="showDetailsDialog()" class="modal-overlay" (click)="closeDetailsDialog()">
      <div class="modal-content details-modal" (click)="$event.stopPropagation()">
        <div class="modal-header">
          <h2>Detalles de Estimación de Puntos de Función</h2>
          ...
          <div class="detail-section">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item">
                ...
                <span [class]="'complexity-badge ' + selectedEstimation()!.complexityLevel.toLowerCase()">
                  {{ getComplexityLabel(selectedEstimation()!.complexityLevel) }}
                </span>
              </div>
              <div class="detail-item">
            ...
            <div class="components-grid">
              <div class="component-card">
                ...
                <div class="component-content">
                  <div class="component-label">Entradas Externas (EI)</div>
                  <div class="component-value">{{ selectedEstimation()!.externalInputs }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">📤</div>
                <div class="component-content">
                  <div class="component-label">Salidas Externas (EO)</div>
                  <div class="component-value">{{ selectedEstimation()!.externalOutputs }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">🔍</div>
                <div class="component-content">
                  <div class="component-label">Consultas Externas (EQ)</div>
                  <div class="component-value">{{ selectedEstimation()!.externalInquiries }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">🗄️</div>
                <div class="component-content">
                  <div class="component-label">Archivos Lógicos Internos (ILF)</div>
                  <div class="component-value">{{ selectedEstimation()!.internalLogicalFiles }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">🔗</div>
                <div class="component-content">
                  <div class="component-label">Archivos de Interfaz Externa (EIF)</div>
                  <div class="component-value">{{ selectedEstimation()!.externalInterfaceFiles }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div class="metrics-grid">
              <div class="metric-box">
                <div class="metric-label">UFP (Sin Ajustar)</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.unadjustedFp) }}</div>
              </div>
              <div class="metric-box">
                <div class="metric-label">Factor de Ajuste</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.valueAdjustmentFactor) }}</div>
              </div>
              <div class="metric-box highlight">
                <div class="metric-label">AFP (Ajustados)</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.adjustedFp) }}</div>
              </div>
            </div>
          </div>
...
export class FunctionPointEstimationsComponent implements OnInit {
  ...
  saveEstimation(data: any) {
    ...
    if (estimation) {
      const request: UpdateFunctionPointEstimationRequest = {
        fpEstimationId: estimation.fpEstimationId,
        projectId: estimation.projectId,
        ...data
      };

      this.fpService.updateEstimation(estimation.fpEstimationId, request).subscribe({
        next: (response) => {
          if (response.success && response.data) {
            this.estimations.update(list => 
              list.map(e => e.fpEstimationId === response.data!.fpEstimationId ? response.data! : e)
            );
      ...
      const request: CreateFunctionPointEstimationRequest = {
        projectId: project.projectId,
        ...data
      };

      this.fpService.createEstimation(request).subscribe({
        next: (response) => {
          if (response.success && response.data) {
  ...
  editEstimation(estimation: FunctionPointEstimation) {
    this.selectedEstimation.set(estimation);
    this.selectedProject.set(null);
    this.showEstimationDialog.set(true);
  }

  deleteEstimation(estimation: FunctionPointEstimation) {
    if (!confirm(`¿Estás seguro de eliminar la estimación "${estimation.estimationName}"?`)) {
      return;
    }

    this.fpService.deleteEstimation(estimation.fpEstimationId).subscribe({
      next: (response) => {
        if (response.success) {
          this.estimations.update(list => 
            list.filter(e => e.fpEstimationId !== estimation.fpEstimationId)
          );
        } else {
  ...
  viewEstimation(estimation: FunctionPointEstimation) {
    this.selectedEstimation.set(estimation);
  ...
  getTotalFP(): number {
    return this.estimations().reduce((sum, e) => sum + (e.adjustedFp || 0), 0);
  }
