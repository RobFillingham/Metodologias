...
import { UseCasePointEstimationDialogComponent } from '../use-case-point-estimation-dialog/use-case-point-estimation-dialog.component';
import { UseCasePointEstimationService } from '../../../core/services/use-case-point/use-case-point-estimation.service';
...
} from '../../../core/models/use-case-point/use-case-point.models';

@Component({
  selector: 'app-use-case-point-estimations',
  standalone: true,
  imports: [
    CommonModule, 
    NavbarComponent, 
    SelectProjectDialogComponent,
    UseCasePointEstimationDialogComponent
  ],
  template: `
    ...
    <div class="container">
      <div class="header">
        <div class="header-content">
          ...
          <h1>Estimaciones de Puntos de Caso de Uso</h1>
          <p class="subtitle">Gestión de estimaciones basadas en Use Case Points (UCP)</p>
        </div>
        ...
        <table class="estimations-table">
          ...
          <tbody>
            <tr *ngFor="let estimation of estimations()" class="estimation-row">
              ...
              <td>{{ formatDecimal(estimation.unadjustedUcp) }}</td>
              <td>
                <span class="ucp-badge">{{ formatDecimal(estimation.adjustedUcp) }}</span>
              </td>
              <td>{{ formatDecimal(estimation.estimatedEffort) }}</td>
              <td>{{ formatDecimal(estimation.estimatedEffortPm) }}</td>
              <td>{{ formatDecimal(estimation.estimatedTime) }} meses</td>
      ...
      <div *ngIf="!loading() && !error() && estimations().length > 0" class="statistics">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            <div class="stat-icon">🎯</div>
            <div class="stat-content">
              <div class="stat-value">{{ formatDecimal(getTotalUCP()) }}</div>
              <div class="stat-label">Total Puntos UCP</div>
            </div>
          </div>
          <div class="stat-card">
            ...
            <div class="stat-content">
              ...
              <div class="stat-label">Total Esfuerzo (horas)</div>
            </div>
          </div>
          <div class="stat-card">
            <div class="stat-icon">💰</div>
    ...
    <!-- Estimation Dialog -->
    <app-use-case-point-estimation-dialog
    ...
    <div *ngIf="showDetailsDialog()" class="modal-overlay" (click)="closeDetailsDialog()">
      <div class="modal-content details-modal" (click)="$event.stopPropagation()">
        <div class="modal-header">
          <h2>Detalles de Estimación UCP</h2>
            ...
            <div class="components-grid">
              <div class="component-card">
                ...
                <div class="component-content">
                  <div class="component-label">Simples (peso 5)</div>
                  <div class="component-value">{{ selectedEstimation()!.simpleUccCount }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">🟡</div>
                <div class="component-content">
                  <div class="component-label">Promedio (peso 10)</div>
                  <div class="component-value">{{ selectedEstimation()!.averageUccCount }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">🔴</div>
                <div class="component-content">
                  <div class="component-label">Complejos (peso 15)</div>
                  <div class="component-value">{{ selectedEstimation()!.complexUccCount }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div class="components-grid">
              <div class="component-card">
                <div class="component-icon">👤</div>
                <div class="component-content">
                  <div class="component-label">Simples (peso 1)</div>
                  <div class="component-value">{{ selectedEstimation()!.simpleActorCount }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">👥</div>
                <div class="component-content">
                  <div class="component-label">Promedio (peso 2)</div>
                  <div class="component-value">{{ selectedEstimation()!.averageActorCount }}</div>
                </div>
              </div>
              <div class="component-card">
                <div class="component-icon">👨‍👩‍👧‍👦</div>
                <div class="component-content">
                  <div class="component-label">Complejos (peso 3)</div>
                  <div class="component-value">{{ selectedEstimation()!.complexActorCount }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div class="metrics-grid">
              <div class="metric-box">
                <div class="metric-label">UUCP (Sin Ajustar)</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.unadjustedUcp) }}</div>
              </div>
              <div class="metric-box">
                <div class="metric-label">Factor Técnico</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.technicalComplexityFactor) }}</div>
              </div>
              <div class="metric-box">
                <div class="metric-label">Factor Ambiental</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.environmentFactor) }}</div>
              </div>
              <div class="metric-box highlight">
                <div class="metric-label">UCP Ajustado</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.adjustedUcp) }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div class="metrics-grid">
              <div class="metric-box highlight">
                ...
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.estimatedEffort) }}</div>
              </div>
              <div class="metric-box highlight">
                <div class="metric-label">Esfuerzo (PM)</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.estimatedEffortPm) }}</div>
              </div>
              <div class="metric-box highlight">
                <div class="metric-label">Tiempo (meses)</div>
                <div class="metric-value">{{ formatDecimal(selectedEstimation()!.estimatedTime) }}</div>
              </div>
              <div class="metric-box highlight">
...
export class UseCasePointEstimationsComponent implements OnInit {
  private ucpService = inject(UseCasePointEstimationService);
  private projectService = inject(ProjectService);
  private router = inject(Router);

  estimations = signal<UseCasePointEstimation[]>([]);
  ...
  selectedEstimation = signal<UseCasePointEstimation | null>(null);
      ...
      next: (response) => {
        if (response.success && response.data) {
          ...
          const estimationRequests = userProjects.map(project =>
            this.ucpService.getEstimationsByProject(project.projectId).pipe(
              catchError(err => {
                console.error(`Error loading estimations for project ${project.projectId}:`, err);
                return of({ success: false, data: [] as UseCasePointEstimation[], message: '' });
              })
            )
          );
          ...
          forkJoin(estimationRequests).subscribe({
            next: (responses) => {
              const allEstimations: UseCasePointEstimation[] = [];
  ...
  saveEstimation(data: any) {
    ...
    if (estimation) {
      const request: UpdateUseCasePointEstimationRequest = {
        ucpEstimationId: estimation.ucpEstimationId,
        projectId: estimation.projectId,
        ...data
      };

      this.ucpService.updateEstimation(estimation.ucpEstimationId, request).subscribe({
        next: (response) => {
          if (response.success && response.data) {
            this.estimations.update(list => 
              list.map(e => e.ucpEstimationId === response.data!.ucpEstimationId ? response.data! : e)
            );
      ...
      const request: CreateUseCasePointEstimationRequest = {
        projectId: project.projectId,
        ...data
      };

      this.ucpService.createEstimation(request).subscribe({
        next: (response) => {
          if (response.success && response.data) {
  ...
  editEstimation(estimation: UseCasePointEstimation) {
    this.selectedEstimation.set(estimation);
    this.selectedProject.set(null);
    this.showEstimationDialog.set(true);
  }

  deleteEstimation(estimation: UseCasePointEstimation) {
    if (!confirm(`¿Estás seguro de eliminar la estimación "${estimation.estimationName}"?`)) {
      return;
    }

    this.ucpService.deleteEstimation(estimation.ucpEstimationId).subscribe({
      next: (response) => {
        if (response.success) {
          this.estimations.update(list => 
            list.filter(e => e.ucpEstimationId !== estimation.ucpEstimationId)
          );
        } else {
  ...
  viewEstimation(estimation: UseCasePointEstimation) {
    this.selectedEstimation.set(estimation);
  ...
  getTotalUCP(): number {
    return this.estimations().reduce((sum, e) => sum + (e.adjustedUcp || 0), 0);
  }
