...
/// <summary>
/// Service implementation for COCOMO II Stage 3 calculation operations
...
public class CocomoCalculationService : ICocomoCalculationService
{
    ...
    public decimal CalculateEAF(Estimation estimation, ParameterSet parameterSet)
    {
        ...
        // RELY
        eaf *= GetEMValue(parameterSet, "RELY", estimation.SelectedEmRely);

        // DATA
        eaf *= GetEMValue(parameterSet, "DATA", estimation.SelectedEmData);

        // CPLX
        eaf *= GetEMValue(parameterSet, "CPLX", estimation.SelectedEmCplx);

        // RUSE
        eaf *= GetEMValue(parameterSet, "RUSE", estimation.SelectedEmRuse);

        // DOCU
        eaf *= GetEMValue(parameterSet, "DOCU", estimation.SelectedEmDocu);

        // TIME
        eaf *= GetEMValue(parameterSet, "TIME", estimation.SelectedEmTime);

        // STOR
        eaf *= GetEMValue(parameterSet, "STOR", estimation.SelectedEmStor);

        // PVOL
        eaf *= GetEMValue(parameterSet, "PVOL", estimation.SelectedEmPvol);

        // ACAP
        eaf *= GetEMValue(parameterSet, "ACAP", estimation.SelectedEmAcap);

        // PCAP
        eaf *= GetEMValue(parameterSet, "PCAP", estimation.SelectedEmPcap);

        // PCON
        eaf *= GetEMValue(parameterSet, "PCON", estimation.SelectedEmPcon);

        // APEX
        eaf *= GetEMValue(parameterSet, "APEX", estimation.SelectedEmApex);

        // PLEX
        eaf *= GetEMValue(parameterSet, "PLEX", estimation.SelectedEmPlex);

        // LTEX
        eaf *= GetEMValue(parameterSet, "LTEX", estimation.SelectedEmLtex);

        // TOOL
        eaf *= GetEMValue(parameterSet, "TOOL", estimation.SelectedEmTool);

        // SITE
        eaf *= GetEMValue(parameterSet, "SITE", estimation.SelectedEmSite);
    ...
    {
        // Convert to PascalCase: "RELY" -> "Rely", "NOM" -> "Nom"
        var factorPascal = ToPascalCase(factor);
        var ratingPascal = ToPascalCase(rating);
